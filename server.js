    ...
    try {
        ...
        for (const file of req.files) {
            ...
            if (file.mimetype.startsWith('image/')) {
                try {
                    const thumbnailPath = `uploads/images/thumb_${file.filename}`;
...
app.delete('/api/file/:fileId', (req, res) => {
    ...
    try {
        ...
        if (fileInfo.thumbnail && fs.existsSync(fileInfo.thumbnail)) {
            fs.unlinkSync(fileInfo.thumbnail);
        }
    } catch (error) {
    ...
    for (const [fileId, fileInfo] of files.entries()) {
        ...
        if (fileAge > maxAge) {
            ...
            try {
                ...
                files.delete(fileId);
                console.log(`Deleted old file: ${fileInfo.originalName}`);
            } catch (error) {
